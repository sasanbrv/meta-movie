        ...
        <div onClick={onClick}
            ...
            <div className="p-4">
                ...
                <div className="mt-2 flex justify-between">
                    <span className="flex text-violet items-center gap-2"><FaStar className="text-yellow-500"/>{movie.vote_average.toFixed(1)}</span>
                    <span className="text-violet">{movie.release_date?.split("-")[0] || movie.first_air_date?.split("-")[0]}</span>
                </div>
            </div>
        </div>
        </>
